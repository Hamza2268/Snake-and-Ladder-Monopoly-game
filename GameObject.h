...
public:
	...
	virtual void Save(std::ofstream& outfile, ObjectType type) = 0;	// Saves the GameObject parameters to the file
	virtual void Load(ifstream &Infile) = 0;	// Loads and Reads the GameObject parameters from the file
	virtual ObjectType GetType() const;  // virtual function to gettype of stored object

	virtual bool IsOverlapping(GameObject* newObj) const = 0; //pure virtual function to check overlapping
