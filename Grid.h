...
// ========= Setters and Getters Functions =========
	...
	bool SetGameObjectInCell(const CellPosition& pos, GameObject* pGameObject);
	GameObject* GetGameObjectFromCell(const CellPosition& position) const;
	...
	//void SaveAll(ofstream& OutFile, GameObject::Type t);

	// Function to check if a cell is valid for the start of a ladder
	bool IsCellValidForStart(const CellPosition& startPos) const;


	// Function to check if the end cell is the start of another ladder or snake
	bool IsEndCellStartOfAnotherObject(const CellPosition& endPos) const;

	void SaveAll(std::ofstream& outfile, ObjectType type); //Function to Save all objects to file

	void ClearGrid();

	void LoadAll(std::ifstream& infile, ObjectType type); //Function to Load all objects from file

	int CountGameObjects(ObjectType type) const;

	bool IsOverlapping(GameObject* newObj) const; // to loop on all cells to check the overlapping
	...
	int SelectTargetPlayer(int CurrentPlayer);
	void ApplySpecialAttack(Player& player, const std::string& attack, int CurrentPlayer);// to Apply the special attacks
